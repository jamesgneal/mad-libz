...
var db = require("../models"),
	madLib = db.MadLib,
    story = db.Story,
    CompletedStory = db.CompletedStory;
...
router.post("/api/madlibz/completedstories", function(req, res) {
	//posting path for stories.
	console.log(req.body);
	CompletedStory.create({
		newStory: req.body.newStory
	}).then(function(result) {
...
router.get("/random", function(req, res) {
		CompletedStory.find({ order: [Sequelize.fn("RAND")] }).then(function(result) {
			//capture the words of the story that was randomly found
			var storyPlaceholder = result.dataValues.newStory;
            ...
            res.render("random", handleBarsObj);
